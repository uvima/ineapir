...
paro <- get_data_table(idTable = 4247, filter = filter, nlast = 1,
                       unnest = TRUE, metanames = TRUE, tip = "AM")
...
# Table url: https://www.ine.es/jaxiT3/Tabla.htm?t=4247&L=0
paro <- get_data_table(idTable = 4247, filter = filter, nlast = 1,
                       unnest = TRUE, metacodes = TRUE, tip = "AM")

# Select a set on columns from data
paro <- subset(paro, select = c("Comunidades.y.Ciudades.Autónomas.Id", "T3_Periodo",
                                "Anyo","Valor"))

# Join boundaries information with data information
ccaa <- merge(ccaa, paro, by.x = "id_region", 
